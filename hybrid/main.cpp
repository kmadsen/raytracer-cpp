...
RenderContext* contextThread1;
RenderContext* contextThread2;
RenderContext* contextThread3;
RenderContext* contextThread4;
...
unsigned __stdcall drawThread(void* args)
{
...
void drawObjects(Vector lookdir)
{
	...
	if (hybridOn && !multiThreadOn) {
	...
	else if (hybridOn && multiThreadOn) {	
		HANDLE hThreads[4];
		unsigned threadsID[4];
		hThreads[0] = (HANDLE)_beginthreadex(NULL, 0, drawThread, (void*)contextThread1, 0, &threadsID[0] );
		hThreads[1] = (HANDLE)_beginthreadex(NULL, 0, drawThread, (void*)contextThread2, 0, &threadsID[1] );
		hThreads[2] = (HANDLE)_beginthreadex(NULL, 0, drawThread, (void*)contextThread3, 0, &threadsID[2] );
		hThreads[3] = (HANDLE)_beginthreadex(NULL, 0, drawThread, (void*)contextThread4, 0, &threadsID[3] );
		...
		//scene->addShadowsHybrid(stencils, pixels);
		//scene->renderRasterizedScene();
	}
}
...
void display( )
{
	...
	if (sceneIsDirty)// || !hybridOn)
		...
		// update the threading contexts
	int resX = scene->getImage()->getXresolution();
	int resY = scene->getImage()->getYresolution();
	int resXInc = resX / 4;
	contextThread1->setBounds(resXInc*0, 0, resXInc*1, resY);
	contextThread2->setBounds(resXInc*1, 0, resXInc*2, resY);
	contextThread3->setBounds(resXInc*2, 0, resXInc*3, resY);
	contextThread4->setBounds(resXInc*3, 0, resX     , resY);
...
int main( int argc, char** argv )
{
	...
	contextThread1 = new RenderContext(scene);
	contextThread2 = new RenderContext(scene);
	contextThread3 = new RenderContext(scene);
	contextThread4 = new RenderContext(scene);
